#I will like to acknowledge Samata Dutta and Ryuichiro Izumi for guidance with the 
#codes and other computational issues
...
for (i in 1:5000){
  B11[i] <- sum(abs(diff(X[1:77,i])/n)*abs(diff(X[2:78,i])/n))
}
B11<- B11*(delta/(m1^2))

B1111<-NULL
for (i in 1:5000){
  B1111[i] <- sum(abs(diff(X[1:75,i])/n)*abs(diff(X[2:76,i])/n)*abs(diff(X[3:77,i])/n)*abs(diff(X[4:78,i])/n))
}
...
t <- seq(0, 1/252, by = ((1/252)/77))
sigma=matrix(data=0, nrow=78, ncol=5000)
dsigma=matrix(data=0, nrow=78, ncol=5000)
dX=dsigma
RES=NULL

RES=lapply(1:5000, function(k){
  P=rep(0,77)
  dB <- BM(x = 0, t0 = 0, T = 1/252, N = 78)
  dW <- BM(x = 0, t0 = 0, T = 1/252, N = 78)
  for (l in 1:length(P)){
...
for (i in 1:5000){
  B11_2[i] <- sum(abs(diff(X2[1:77,i])/n)*abs(diff(X2[2:78,i])/n))
}
B11_2<- B11_2*(delta/(m1^2))

B1111_2<-NULL
for (i in 1:5000){
  B1111_2[i] <- sum(abs(diff(X2[1:75,i])/n)*abs(diff(X2[2:76,i])/n)*abs(diff(X2[3:77,i])/n)*abs(diff(X2[4:78,i])/n))
}
...
X3 <- matrix(data = 0, nrow = 78, ncol = 5000)
sigma3 <- X3
ni <- X3
...
sim_X3 <- matrix(0, nrow = 78, ncol = 5000)
sim_sigma3 <- sim_X3
sim_ni <- sim_X3
...
for (i in 1:5000){
  B11_3[i] <- sum(abs(diff(sim_X3[1:77,i])/n)*abs(diff(sim_X3[2:78,i])/n))
}
B11_3<- B11_3*(delta/(m1^2))

B1111_3<-NULL
for (i in 1:5000){
  B1111_3[i] <- sum(abs(diff(sim_X3[1:75,i])/n)*abs(diff(sim_X3[2:76,i])/n)*abs(diff(sim_X3[3:77,i])/n)*abs(diff(sim_X3[4:78,i])/n))
}
...
# Interation Procedure to Estimate IV
